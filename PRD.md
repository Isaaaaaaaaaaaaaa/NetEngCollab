...
- 人员概览：查看全部老师/学生账号信息，按角色筛选，支持按学号/工号或姓名搜索，侧边栏按姓名快速定位，支持创建账号、批量导入与重置密码，并在详情中展示学生画像摘要/教师项目统计。
- 项目概览：查看全部项目详情、发布教师、所有选择该项目的学生及状态，支持按项目类型（科研/大创/竞赛）与项目名称搜索，并允许管理员重置某个学生的选择状态以便重新双选。
- 数据分析：查看平台活跃度折线图、热门研究方向标签、竞赛参与趋势等统计图表。
...
- 项目浏览与匹配：筛选条 + 列表；支持按类型、教师、点赞/收藏/我加入等维度筛选；项目详情中展示“申请状态/邀请状态”。
- 合作项目进展：学生在“合作项目进展”页面查看已确认合作项目的里程碑与进度更新，并可提交自己的进展记录。
- 邀请处理：在工作台展示“合作邀请”卡片，支持接受/拒绝。

#### 2.4.4 队员 3：管理员端关键页面

- 管理员导航包括：总览、人员概览、项目概览、数据分析。
- 人员概览：左侧姓名列表，右侧详情面板；支持按角色筛选和按学号/工号或姓名搜索；支持创建账号、批量导入与重置密码。
- 项目概览：左侧项目列表（支持分页、类型筛选与按项目名称搜索），右侧项目详情与选择学生状态表，管理员可重置单个学生的选择状态。
- 数据分析：专门展示平台活跃度曲线、研究热门方向标签云和竞赛参与趋势。
...
- 学生画像：`GET/PUT /api/student-profile` 维护专业、年级、班级、技能、兴趣、周投入时间、可见性等字段，并为学生匹配与管理员人员概览提供数据支撑。
